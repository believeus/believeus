        ...
        success: function(cateList){
            if(cateList) {
                ...
                $("div.nav > ul > li").each(function(i,obj){
                    ...
                    if(key == 'lastSelected') {
                        //有key的标签切换，无则是所有分类
                         innerHTML = innerAll(cateList['all']);
                    }else{
                         innerHTML = innerTag(cateList['tag'][i]);
                    }
...
$(function(){     
    ...
    // 取到滚动条高度，高度>500发生show事件
	    if($(window).scrollTop()>=110&&$(window).scrollTop()<950){
	    	$("#nav").css("top",$(window).scrollTop()-120);
	    }
	    else if($(window).scrollTop()<=100){
	    	$("#nav").css("top",0);
	    }else if($(window).scrollTop()>=960){
	    	$("#nav").css("top",900);
	    }
    });
  });