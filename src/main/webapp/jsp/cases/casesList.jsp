...
<c:forEach items="${casesList }" var="cases" varStatus="s">
...
<!-- End Name:wuqiwei Date:2013-08-14 AddReason:该逻辑是用来对内容框的每行三列的显示 -->
    <div>
	  <div style="position:absolute; left:${260+((s.index mod 3)*280)}px;top: ${top*350+70 }px;">
		...
		<div style="top:-245px; position: relative; left: 50px; "><a href="/casesDetail-id-${cases.id}.jhtml" style="color: red;"><html:html value="${cases.title}" num="8"/></a></div>
		<div style="top: -240px; position: relative; left: 30px; font-size: 14px; width: 180px; height: 20px;"><a> <html:html value="${cases.content }" num="100"/></a></div>
	  </div>
  </div>
</c:forEach>  
</div>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			...
			if(count>countPerPage){
				var believeusPagiation=new BelieveusPagiation('Searchresult','/casesAjax',countPerPage,count,"jhtml");
