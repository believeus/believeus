...
<body>
		...
		<tr>
			<td colspan="10" style="text-align:left; text-indent:1em">
				<form id="alertform" name="alertform" method="post" action="">
					<font style="color: green; font-size: 20px;">药膳信息搜索：</font> 标题:<span>
	...
	<!-- 分页的显示区 -->
	 	<table class="tableborder" border="0" id="tbody">
			...
			<a href="/adminNewYaoShanView.jhtml"><font style="font-size: 15px;">新建药膳</font></a>
				<span class="r">搜索结果：<b></b>个
			</span> <span><a href="">全部药膳信息</a><b>${bowenCount}</b>个 <a href="">封禁</a><b>2000</b>个
			</span></td>
		</tr>
		<tr>
			<th>药膳编号</th>
			...
			<c:forEach items="${yaoShanList}" var="yaoShan" varStatus="status">
			<!-- Begin Name:wuqiwei Date 2013-4-17 21:10:13 addReason:奇偶行变色 -->
				...
				<tr id="_Item_Index_1" align="left" style="max-height: 50px; background-color: ${backgroundColor}">
					<td style="text-align: left;width: 80px;"> 
						<input type="checkbox" name="infoIds" value="${yaoShan.id}" />
						<label>${yaoShan.id}</label>
					</td>
					<td style="text-align: left; max-width: 50px;">
							<a title="${yaoShan.title}" href="">${yaoShan.title }</a>
					</td>
					<td>
						${yaoShan.author }
					</td>
					<td style="max-width: 100px; text-align:left; max-height: 50px;">
						<a href="/adminYaoShanDetail-Jid-${yaoShan.id}.jhtml" >
							<html:html num="45" value="${yaoShan.content }"></html:html>
						</a>
					</td>
					<td style="max-width: 60px; text-align: left; text-align:justify;">

						<!-- begin Name:wuqiwei Date:2013-4-13 这是自定义的标签库，使用来将long类型转换成时间的 -->
						<date:date value="${yaoShan.editDate }" parttern="yyyy-MM-dd HH:mm:ss E" />
						<!-- begin Name:wuqiwei Date:2013-4-13 这是自定义的标签库，使用来将long类型转换成时间的 -->
					</td>
					<td id="baz1Item877">
						<c:choose>
							<c:when test="${yaoShan.status eq 0 }"><font color="green">正常</font></c:when>
							<c:when test="${yaoShan.status eq 1 }"><font color="red">封禁</font></c:when>
							<c:when test="${yaoShan.status eq 2 }"><font color="yellow">推荐</font></c:when>
						</c:choose>
					</td>
					<td>
						<a href="/adminYaoShanEdit-id-${yaoShan.id}-currpage-${currentPage}.jhtml">编辑</a>|
						<a name="confirm_button" style="cursor: pointer; color: red;" url="/adminDeleteYaoShan-id-${yaoShan.id}-currPage-${currentPage}.jhtml">删除</a>
					</td>
				</tr>
				</c:forEach>
			</table>
	...
	<script type="text/javascript">
		$(document).ready(function(){
			var count="${yaoShanCount}";
			var countPerPage=5;
			if(count>countPerPage){
				var believeusPagiation=new BelieveusPagiation('Searchresult','/adminSubYaoShanAjaxList',countPerPage,count,"jhtml");
